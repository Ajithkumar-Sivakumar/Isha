...
Common issues and quick fixes when running the stack locally.

- **ERR_CONNECTION_REFUSED when opening Grafana (http://localhost:3000)**:
  - Ensure the stack is running: `docker compose ps`
  - Start or restart services: `docker compose up -d --build`
  - Check Grafana logs: `docker compose logs -f grafana`
  - If the port is already in use, stop the conflicting process or change the port mapping in `docker-compose.yml`.

- **curl http://localhost:5000/health returns connection refused**:
  - Confirm the API process is running locally: `lsof -iTCP:5000 -sTCP:LISTEN -Pn`
  - If running via the repository Python entrypoint: activate the venv and start the app:

```bash
cd ObservabilityShipping_Python
python3 -m venv venv
. venv/bin/activate
...
- **Endpoints return 5xx or DB connection errors**:
  - Verify Postgres is available when using the `development` profile (default `DATABASE_URL`):
    - If you use Docker Compose, the Postgres service is included and will be started by `docker compose up -d`.
    - Check Postgres logs: `docker compose logs -f postgres`
  - Alternatively run the tests with an in-file SQLite DB: `FLASK_ENV=test pytest`.

- **`/metrics` missing or empty**:
  - Ensure `prometheus-client` is installed (`pip install prometheus-client`) or run `pip install -r requirements.txt`.
  - Metrics are disabled for paths `/health` and `/metrics` by design; they will not be recorded in the request-duration histogram.

- **Useful commands**:

```bash
# View API logs (if running with docker compose)
docker compose logs -f api

# Check all containers and ports
docker compose ps

# Quick local port check
lsof -iTCP -sTCP:LISTEN -Pn | grep -E "5000|3000|9090"
```

If you'd like, I can add container-specific troubleshooting to the repo README or create a dedicated `TROUBLESHOOTING.md` with expanded checks and common log excerpts.
